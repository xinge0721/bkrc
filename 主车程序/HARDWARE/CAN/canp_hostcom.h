...
static uint8_t Send_zigbee[8];                                    //全局Zigbee发送数组
static uint8_t Send_Infrared[6];                                  //全局Infrared发送数组
...
GLOBAL uint16_t  Get_Host_UpTrack( uint8_t mode);
...
GLOBAL void Send_InfoData_To_Fifo( char  *p ,uint8_t len);
GLOBAL void Set_Track_Pwr( uint16_t power);
