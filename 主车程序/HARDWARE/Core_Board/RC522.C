...
uint8_t WRITE_RFID[16]={"0123456789ABCDEF"};//写入RFID卡的数据
				...
				if(PcdAuthState(KEYA,card_key,KEY_A,SN) == MI_OK)	// 验证密钥（A密钥）
				{
					...
					if((mode == RFID_Read) || (mode == RFID_Write_Read))	// 允许读出数据
					{
						if(PcdRead(card_addr,READ_RFID) == MI_OK)			// 读取数据
						{
							...
							Send_InfoData_To_Fifo((char *)READ_RFID,16);//打印到屏幕上面—debug区域
...
//循迹读卡停车
void RFID_car_stop(void)
{
while(1)
		{
			if(RFID_Flag==0)
			{
				Track_duoy_RFID(70,95);//循迹
			}
				
			if(RFID_Flag==1)
			{
				Forward(70,95,270);  //十字路口前进一点 270
			...
			if(PcdRequest(PICC_REQALL,CT) == MI_OK)   //寻到RFID
			{
				 Roadway_Flag_clean();	//清除标志位状态	
         RC522(1,RFID_Read);//读卡  
				 if(RFID_Flag==3  )//读出数据
				 {
					RFID_Flag=0;
					break;
